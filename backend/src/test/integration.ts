...
import { creatAppContext } from '../lib/ctx'
...
if (env.NODE_ENV !== 'test') {
  throw new Error('You must run tests with NODE_ENV=test')
}

export const appContext = creatAppContext()
...
export const getTrpcCaller = (user?: User) => {
  ...
  const ctx = getTrpcContext({ appContext, req })
